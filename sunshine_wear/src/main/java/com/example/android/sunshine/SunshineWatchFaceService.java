...
public class SunshineWatchFaceService extends CanvasWatchFaceService {
    ...
    private class Engine extends CanvasWatchFaceService.Engine implements
        ...
        public void onDraw(Canvas canvas, Rect bounds) {
            ...
            if (mHighTemp != null && mLowTemp != null) {
                ...
                if (mAmbient) {
                    ...
                    canvas.drawText(mLowTemp, xOffset + highTextSize , mYOffsetWeather, mTextTempLowPaint);
                } else {
                    mTextTempLowPaint.setColor(getResources().getColor(R.color.primary_light));
                    float xOffset = bounds.centerX() - (highTextSize / 2);
                    canvas.drawText(mHighTemp, xOffset, mYOffsetWeather, mTextTempHighPaint);
                    canvas.drawText(mLowTemp, bounds.centerX() + (highTextSize / 2) , mYOffsetWeather, mTextTempLowPaint);

                    // Draw icon (if not in ambient mode)
                    Drawable b = getResources().getDrawable(Utility.getSmallArtResourceIdForWeatherCondition(mWeatherId));
        ...
        public void onDataChanged(DataEventBuffer dataEventBuffer) {
            Log.d("WATCH_DATA","onDataChanged Called");
